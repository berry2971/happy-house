...
echo [93mCopy raw data to MySQL local upload directory...[0m
copy ..\raw\*.csv %LOCAL_DATA_PATH%
echo [93mCopied raw data to MySQL local upload directory successfully.[0m
...
mysql -u %ID% -p%PW% -P %PORT% < ../raw/ddl.sql
...
echo|set /p=districts.csv >> lib\temp\cur.sql
...
echo [93mImport complexes...[0m

type lib\frag1.fsql > lib\temp\cur.sql
echo|set /p=complexes.csv >> lib\temp\cur.sql
